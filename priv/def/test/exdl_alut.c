 ...
 *
 *     $Id: exdl_alut.c,v 1.5 2005/06/29 14:45:38 dgud Exp $
 */
...
EXDL_API(ealut_loadMemoryFromFile)
{
  ...
  STACK_FMT_BEGIN()
    STACK_FMT_LPTR(char*,    fileName)
    STACK_FMT_LPTR(ALenum*,  format)
    STACK_FMT_LPTR(ALsizei*, size)
    STACK_FMT_DATA(ALfloat,  frequency[])
  STACK_FMT_END()
  ...
  CATCH_RESULT(ALvoid*)
    ...
    ( STACK_GET_DATA(fileName)
    , STACK_GET_DATA(format)
    , STACK_GET_DATA(size)
    , STACK_GET_ADDR(frequency)
    );

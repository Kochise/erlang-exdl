...
%% 
%%     $Id: exdl_video.erl,v 1.5 2006/06/26 12:10:01 dgud Exp $
%%
...
%%% Modified : 26 Nov 2008 by David Koch <david.koch@libertysurf.fr>
%%%----------------------------------------------------------------------
...
; listModes(PF, Flags)
  when is_record(PF, sdl_pixelformat)
 -> <<M:8
    , Bin/binary
    >>
    = call
...
%% C-API func: int SDL_BlitSurface(SDL_Surface* src, SDL_Rect* srcrect, SDL_Surface* dst, SDL_Rect* dstrect);

blitSurface(SSurf, SRect, DSurf, DRect) 
  when is_record(SSurf, sdl_surface)
 -> blitSurface
    ( SSurf#sdl_surface.self
    , SRect
    , DSurf
    , DRect
    )
...
; blitSurface({surfacep, SrcRef}, SRect, {surfacep, DestRef}, DRect)
 -> blitSurface2
    ( <<SrcRef:?_PTR
      , DestRef:?_PTR
      , 3:8
...
; setColors({surfacep, Ref}, Colors, FirstColor, Ncolors)
  when is_list(Colors)
 -> BinC
    = list_to_binary
      ( mkbin_colors
...
wm_setIcon(Icon, Mask)
  when is_record(Icon, sdl_surface)
 -> wm_setIcon
    ( Icon#sdl_surface.self
    , Mask
    )

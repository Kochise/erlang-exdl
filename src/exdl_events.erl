...
%% 
%%     $Id: exdl_events.erl,v 1.1 2004/03/30 07:46:20 bjorng Exp $
%%
 ...
 -> case
    ...
    ; Bin
        -> case
           ...
           of 
             %% Unknown events comes as empty list so we loop until we get an correct event.
             []
               -> waitEvent()
           ; [Event]
               -> Event
...
; decode_events(<<?SDL_KEYDOWN:8, Which:8, State:8, Scancode:8, Sym:16, Mod:16, Unicode:16, Rest/binary>>, Acc)
 -> Event
    = #keyboard
      { which    = Which
      , state    = State
...
; decode_events(<<?SDL_MOUSEMOTION:8, Which:8, State:8, Mod:16, X:16, Y:16, Xrel:16/signed, Yrel:16/signed,Rest/binary>>, Acc)
 -> Event
    = #mousemotion
      { which = Which
      , state = State
...
; decode_events(<<?SDL_MOUSEBUTTONDOWN:8, Which:8, Button:8, State:8, Mod:16, X:16, Y:16,Rest/binary>>, Acc)
 -> Event
    = #mousebutton
      { which  = Which
      , button = Button
...
; decode_events(<<?SDL_JOYBALLMOTION, Which:8, Ball:8, Xrel:16/signed, Yrel:16/signed,Rest/binary>>, Acc)
 -> Event
    = #joyball
      { which = Which
      , ball  = Ball

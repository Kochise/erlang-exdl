...
%% 
%%     $Id: exdl_util.erl,v 1.3 2006/05/15 14:45:44 dgud Exp $
%%
...
-include("exdl_gl.hrl").    % for GL data type
-include("exdl_video.hrl"). % for sdl_surface record
...
readBin(#sdlmem{type=Type, bin=Bin}, Size)
 -> if
      Type =:= ?GLbyte
    ; Type =:= ?GLubyte
        -> %% Force a copy to prevent the read binary to change later.
           <<Data:Size/binary
           , _/binary
           >>
           = Bin
...
%%          Where Type is GL_TYPE existing in gl.hrl (e.g. ?GLdouble)

term2bin(Bin,  _, _)
  when is_binary(Bin)
 -> Bin
; term2bin(List, N, Type)
  when is_list(List)
 -> if
	    length(List) =:= N
        -> list2bin
...
; float_tuple2bin(Tuple, I, N, Acc)
 -> float_tuple2bin
    ...
    , I - 1
    , N
    , [ <<(element(I, Tuple)):N/native-float
        >>
      | Acc
      ]
    )
...
bin2float_1(N, _,  _, Acc)
  when N < 0
 -> Acc
; bin2float_1(N, Sz, Bin, Acc)
 -> <<_:N/binary
    , F:Sz/native-float-unit:8
    , _/binary
    >>
    = Bin
  , bin2float_1
...
bin2unsigned_1(N, _,  _, Acc)
  when N < 0
 -> Acc
; bin2unsigned_1(N, Sz, Bin, Acc)
 -> <<_:N/binary
    , U:Sz/native-unsigned-unit:8
    , _/binary
    >>
    = Bin
  , bin2unsigned_1
...
copySdlImage2GLArray(Image, Mem, Bpp)
  when is_record(Image, sdl_surface)
 -> copySdlImage2GLArray
...
mem_size(?GL_BYTE, Size)
 -> Size
; mem_size(?GL_UNSIGNED_BYTE, Size)
 -> Size
; mem_size(?GL_UNSIGNED_SHORT, Size)
 -> 2 * Size
; mem_size(?GL_SHORT, Size)
 -> 2 * Size
; mem_size(?GL_UNSIGNED_INT, Size)
 -> 4 * Size
; mem_size(?GL_INT, Size)
 -> 4 * Size
; mem_size(?GL_INT, Size)
 -> 4 * Size
; mem_size(?GL_DOUBLE, Size)
 -> 8 * Size
.

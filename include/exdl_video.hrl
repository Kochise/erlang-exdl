...
%% 
%%     $Id: exdl_video.hrl,v 1.1 2004/03/30 07:50:46 bjorng Exp $
%%
...
%%% Modified: 22 Nov 2008 by David Koch <david.koch@libertysurf.fr>
%%%----------------------------------------------------------------------
...
-record( sdl_surface
      ...
      %% , format_version % unsigned int - Private
...
-record( sdl_overlay
       , { self       %%  Ref to this record
         ...
         , w          % int         - Read-only
         , h          % int         - Read-only 
         , planes     % Uint16      - Read-only 
         , pitches    % Uint16* ref - Read-only 
         , pixels     % Uint8** ref  - Read-write 
         , hwfuncs    % private_yuvhwfuncs* ref
         , hwdata     % private_yuvhwdata* ref
         , hw_overlay % boolean Flag: This overlay hardware accelerated?
         }
       ).
...
-define(SDL_GRAB_FULLSCREEN,       2). % Used internally 
...
-define(SDL_YV12_OVERLAY,          16#32315659). %% Planar mode: Y + V + U  (3 planes)
-define(SDL_IYUV_OVERLAY,          16#56555949). %% Planar mode: Y + U + V  (3 planes)
-define(SDL_YUY2_OVERLAY,          16#32595559). %% Packed mode: Y0+U0+Y1+V0 (1 plane)
-define(SDL_UYVY_OVERLAY,          16#59565955). %% Packed mode: U0+Y0+V0+Y1 (1 plane)
-define(SDL_YVYU_OVERLAY,          16#55595659). %% Packed mode: Y0+V0+Y1+U0 (1 plane)
...
-define(SDL_LOGPAL,                16#01).
-define(SDL_PHYSPAL,               16#02).
